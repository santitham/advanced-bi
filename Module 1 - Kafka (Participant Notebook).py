...
# MAGIC ## Participant Notebook (follow the demos, then complete Part B)
# MAGIC
...
# MAGIC * **Part B — Your Lab (LAB 0, LAB 1, LAB 2, Scenario SC1).** You build it: fill in every `# TODO`
# MAGIC   line, then run the cell. Ask a facilitator if you're stuck — the goal is to reason about the
# MAGIC   mechanism, not to copy code.
# MAGIC
...
# MAGIC
# MAGIC **How to use:** attach to a small cluster, run the two setup cells, follow the demos with the
# MAGIC instructor, then complete the `# TODO`s in Part B (those cells will not run until you fill them in).
...
for i in range(30):
    region = regions[i % 3]
    # TODO: produce a JSON order KEYED BY `region` to TOPIC, with on_delivery=lab_report
    #   hint: lp.produce(TOPIC, key=____, value=make_order(5000 + i), on_delivery=lab_report)
    pass
lp.flush()
...
try:
    for _ in range(40):                   # read ~40 records then stop
        m = c.poll(2.0)
        # TODO: if m is a valid message (m and not m.error()), count its partition:
