...
# MAGIC * **Part C — Your Lab + Incident Simulation.** You build it: fill in every `# TODO` line, then run the
# MAGIC   cell (those cells won't run until you complete them). Ask a facilitator if you're stuck.
# MAGIC
...
# MAGIC
# MAGIC **How to use:** attach to a cluster, run the two setup cells, follow the demos with the instructor,
# MAGIC then complete the `# TODO`s in the Part C lab.
...
# TODO: compute the gold result, build the expected one-row DataFrame, and assert equality.
#   Two orders in category 'A' at price=10 -> total revenue should be 30.0 over 2 orders.
#   got = revenue_by_category(orders, items, price=____)
#   exp = spark.createDataFrame([("A", ____, ____)], "category string, revenue double, orders long")
#   assert_df_equality(got, exp)
#   print("L1 unit test passed")
pass
...
# TODO: assert the gold output has exactly the columns [category, revenue, orders].
#   assert_schema(gold, [____, ____, ____])
#   print("L2 schema test passed")
pass
