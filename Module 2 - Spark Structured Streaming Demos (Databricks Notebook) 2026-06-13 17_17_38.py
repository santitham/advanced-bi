...
# MAGIC
# MAGIC * **Part A — Instructor Demos (D1–D8).** Read the stream → build a DataFrame → the familiar DataFrame API →
# MAGIC   bad-data handling → triggers & monitoring → windowing → watermarks → the Bronze/Silver/Gold pipeline.
# MAGIC   Each maps to a **▶ TRY IT TOGETHER** slide.
# MAGIC * **Part B — Participant Lab + Design Exercise.** Hands-on after the demos. The participant version
# MAGIC   blanks the `# TODO` lines.
# MAGIC
...
BOOT   = "pkc-4j8dq.southeastasia.azure.confluent.cloud:9092"
...
# item -> category lookup (static dimension)
cats = ["Electronics", "Grocery", "Apparel"]
items = spark.createDataFrame([(f"Item_{i}", cats[i % 3]) for i in range(1, 501)],
                              ["itemid", "category"])
...
#                 .agg(_sum("orderunits").alias("revenue"), _count("*").alias("orders")))
lab_rev = ab_rev = (src.withWatermark("event_time", "10 seconds")
                 .groupBy(window(col("event_time"), "10 seconds"))
                 .agg(_sum("orderunits").alias("revenue"), _count("*").alias("orders")))
